...
sample_size = st.sidebar.slider(
    "Historical training sample size",
    min_value=10000,
    max_value=100000,
    value=50000,
    step=10000
...
st.sidebar.write("Upload `flights.csv` below.")

uploaded_file = st.sidebar.file_uploader(
    "Upload historical flights.csv",
    type=["csv"]
)
...
if uploaded_file is None:
    st.info("Upload your `flights.csv` file in the sidebar to begin.")
    st.stop()

with st.spinner("Loading historical data and training model..."):
    df_model = load_data(uploaded_file, sample_size)
...
with col3:
    ...
    st.metric("Historical Delay Rate", f"{delayed_rate:.3f}")
...
with tab2:
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    delay_by_hour.plot(kind="line", marker="o", ax=ax)
...
if st.button("Predict Live Flights", type="primary"):
    ...
    live_predict = pd.DataFrame({
        ...
        "DISTANCE": np.random.choice(df_model["DISTANCE"].dropna(), size=n, replace=True),
    ...
    if len(live_predict) == 0:
        live_predict = pd.DataFrame({
            ...
            "DISTANCE": np.random.choice(df_model["DISTANCE"].dropna(), size=5, replace=True),
